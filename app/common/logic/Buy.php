...
class Buy
{
    ...
    private function _createOrderStep2()
    {
        ...
        if ($post['ifcart']) {
            ...
            $store_cart_list = array();
            $goods_list = array();
            $goods_info['chain_id']=isset($this->_post_data['chain_goods']['0_'.$goods_info['goods_id']])?$this->_post_data['chain_goods']['0_'.$goods_info['goods_id']]:0;
            $goods_list[0] = $store_cart_list[$goods_info['store_id']][0] = $goods_info;
    ...
    private function _createOrderStep4()
    {
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            //自提订单
            $chain_order_data=$order_model->getOrdergoodsList(array(array('order_id','=',$order_id),array('chain_id','>',0)));
            $chain_list=array();
            $chain_model=model('chain');
            $chain_order_model=model('chain_order');
            
            if(count($chain_order_data)>1){//单商品自提
                throw new \think\Exception('自提订单错误[未生成自提数据]', 10006);
            }
            foreach($chain_order_data as $v){
                if(!isset($chain_list[$v['chain_id']])){
                    $chain_list[$v['chain_id']]=$chain_model->getChainOpenInfo(array(array('chain_id','=',$v['chain_id'])));
                }
                if(!$chain_list[$v['chain_id']]){
                    throw new \think\Exception('自提点不存在[未生成自提数据]', 10006);
                }
                if($chain_list[$v['chain_id']]['chain_if_pickup']!=1){
                    throw new \think\Exception('自提点已关闭自提[未生成自提数据]', 10006);
                }
                $chain_order_model->addChainOrder(array(
                    'order_id'=>$v['order_id'],
                    'order_goods_id'=>$v['rec_id'],
                    'chain_order_type'=>2,
                ...
                //减库存
                Db::name('chain_goods')->where(array(array('chain_id','=',$v['chain_id']),array('goods_id','=',$v['goods_id'])))->dec('goods_storage',$v['goods_num'])->update();
            }
    ...
    private function _getGoodsList($cart_list)
    {
        ...
        foreach ($cart_list as $key => $cart) {
            ...
            if (!intval($cart['bl_id'])) {
                //如果是普通商品
                $goods_list[$i]['chain_id'] = isset($this->_post_data['chain_goods']['0_'.$cart['goods_id']])?$this->_post_data['chain_goods']['0_'.$cart['goods_id']]:0;
                ...
                foreach ($cart['bl_goods_list'] as $bl_goods) {
                    $goods_list[$i]['chain_id'] = isset($this->_post_data['chain_goods'][$cart['bl_id'].'_'.$bl_goods['goods_id']])?$this->_post_data['chain_goods'][$cart['bl_id'].'_'.$bl_goods['goods_id']]:0;
                    $goods_list[$i]['goods_num'] = $quantity;
