...
class Buy_1 {
    ...
    public function getStoreFreightList($buy_list = array(), $free_freight_sid_list) {
        ...
        foreach ($buy_list as $key => $goods_info) {
            ...
            //自提商品不需要计算
            if ($goods_info['chain_id']) {
                $freight_list[$goods_info['store_id']] = 0;
                unset($buy_list[$key]);
                continue;
            }
    ...
    public function rcbPay($order_list, $input, $buyer_info) {
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            $order_amount = round(($order_info['order_state'] == ORDER_STATE_DEPOSIT?$order_info['presell_deposit_amount']:$order_info['order_amount']) - $order_info['rcb_amount'] - $order_info['pd_amount'], 2);
            $data_pd = array();
            ...
            if ($available_rcb_amount > 0 && $order_amount>0) {
                ...
                if ($canPay) {//收到货款
                    $logic_order->changeOrderReceivePay($order_list, 'buyer', $member_name);
                }
        return $order_list;
    }
